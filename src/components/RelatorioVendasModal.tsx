  ...
  return (
    <div className="tela-relatorio-cheia" style={{ display: 'flex', zIndex: 400 }}>
      ...
      <div className="corpo-relatorio-cheio" style={{ gap: '16px', display: 'flex', flexDirection: 'column' }}>
        {/* CARDS KPIS */}
        <div className="cards-kpis-container">
          <div className="card-kpi-item">
            <div className="card-kpi-titulo">Total de Vendas</div>
            <div className="card-kpi-valor">
              {vendasConcluidas.length} <span className="card-kpi-sufixo-verde">vendas</span>
            </div>
          </div>

          <div className="card-kpi-item">
            <div className="card-kpi-titulo">Faturamento Real (Líquido)</div>
            <div className="card-kpi-valor valor-verde">
              R$ {faturamentoBruto.toFixed(2).replace('.', ',')}
            </div>
          </div>

          <div className="card-kpi-item">
            <div className="card-kpi-titulo">Vendas Estornadas</div>
            <div className="card-kpi-valor valor-vermelho">
              {vendasEstornadas.length} <span className="card-kpi-sufixo-vermelho">(R$ {valorTotalEstornos.toFixed(2).replace('.', ',')})</span>
            </div>
          </div>
        </div>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '6px' }}>
                      <div style={{ flex: 1, minWidth: '180px' }}>
                        ...
                        <div style={{ fontSize: '0.82rem', color: '#334155', marginTop: '4px', wordBreak: 'break-word', whiteSpace: 'normal' }}>
                          👤 <b>Vendedor/Caixa:</b> {venda.operadorNome}
                        </div>

                        {isEstornada && venda.motivoEstorno && (
                          <div style={{ fontSize: '0.78rem', color: '#991b1b', marginTop: '4px', background: '#fee2e2', padding: '6px 8px', borderRadius: '4px', wordBreak: 'break-word', whiteSpace: 'normal' }}>
                            ⚠️ Motivo do Estorno: {venda.motivoEstorno} (Por: {venda.operadorEstornoNome || 'Supervisor'}) em {venda.dataEstorno}
                          </div>
                        )}
                      </div>

                      <div style={{ fontSize: '1.15rem', fontWeight: 800, color: isEstornada ? '#991b1b' : '#16a34a', flexShrink: 0 }}>
                        R$ {venda.valorTotal.toFixed(2).replace('.', ',')}
                      </div>
                    </div>

                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', width: '100%', borderTop: '1px dashed #e2e8f0', paddingTop: '8px', marginTop: '2px' }}>
                      {onVerCupom && (
                        <button
                          className="btn-acao-rel"
                          style={{ background: '#f1f5f9', color: '#0f172a', fontWeight: 600, flex: 1, minWidth: '70px', padding: '6px 8px', textAlign: 'center', justifyContent: 'center' }}
                      ...
                      <button
                        ...
                        style={{ flex: 1, minWidth: '110px', padding: '6px 8px', textAlign: 'center', justifyContent: 'center' }}
                      ...
                      {!isEstornada && (
                        <button
                          ...
                          style={{ background: '#fee2e2', color: '#dc2626', fontWeight: 700, flex: 1, minWidth: '100px', padding: '6px 8px', textAlign: 'center', justifyContent: 'center' }}
